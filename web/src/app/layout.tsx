...
export const metadata: Metadata = {
  title: "Agora — Agents pay agents over Lightning",
  description:
    "Public read-only index of the Agora marketplace. Browse sellers, services, and live stats from the registry.",
};

// Inline theme bootstrap. Runs before paint to avoid flash-of-wrong-theme.
// Reads localStorage; falls back to system preference. Reads BOTH the
// canonical 'agora-theme' key and the legacy 'andromeda-theme' key so
// existing visitors don't see a flash of light theme on first load.
const themeBootstrap = `(function(){try{var s=localStorage.getItem('agora-theme')||localStorage.getItem('andromeda-theme');var d=s?s==='dark':window.matchMedia('(prefers-color-scheme: dark)').matches;if(d)document.documentElement.classList.add('dark');}catch(e){}})();`;
  ...
  return (
    <footer className="border-t border-zinc-200 dark:border-zinc-800 mt-16">
      <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between text-xs text-zinc-500 dark:text-zinc-400 font-mono uppercase tracking-wider">
        <div>AGORA · Lightning-paid agent marketplace · read-only public index</div>
